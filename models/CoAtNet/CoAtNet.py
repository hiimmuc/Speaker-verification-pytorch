...
from models.CoAtNet.MBConv import MBConvForRelativeAttention
from models.CoAtNet.Transformer import TransformerWithRelativeAttention, ProjectionHead
...
class CoAtNet(nn.Module):
    def __init__(self,  config='coatnet-0', num_classes=None, head_act_fn='mish', head_dropout=0.1, n_mels=64, **kwargs):
        super().__init__()
        ...
        # 80, 102, 1
        inp_h, inp_w, in_channels = n_mels, int(round(nb_samp / hoplength)), 1
...
def MainModel(nOut=512, config=f'coatnet-0', **kwargs):
    
    coatnet = CoAtNet(config=config, num_classes=nOut, **kwargs)
    return coatnet
...
if __name__ == '__main__':
    ...
    config=f'coatnet-0'
    coatnet = CoAtNet(config=config, num_classes=None)
    coatnet.to('cuda:0')
    coatnet.eval()

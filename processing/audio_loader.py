...
class AugmentWAV(object):
    def __init__(self, musan_path, rir_path, max_frames, sample_rate=8000, target_db=None):
        ...
        self.num_noise = {'noise': [1, 1], 'speech': [3, 7], 'music': [1, 1], 'noise_vad': [1,1], 'noise_rirs': [1, 1]}
        self.noiselist = {}
        ...
        noise_vad_files = glob.glob("dataset/augment_data/16kHz/noise_vad/noise_vad/*/*.wav")
        for noise_file in noise_vad_files:
            self.noiselist.setdefault('noise_vad', []).append(noise_file)
        
        # noise from rirs noise
        rir_noise_files = glob.glob(os.path.join('dataset/augment_data/16kHz/RIRS_NOISES/pointsource_noises/', '*.wav')) + glob.glob(os.path.join('dataset/augment_data/16kHz/RIRS_NOISES/real_rirs_isotropic_noises/', '*.wav')) 
